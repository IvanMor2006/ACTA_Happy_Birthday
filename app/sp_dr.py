...
def sp():
    ...
    people_ol = db.execute(GET_PEOPLE_OL).fetchall()
...
def dr(ku):
    ...
    people_ol = db.execute(GET_PEOPLE_OL).fetchall()
    db.close()
    return render_template('sp/dr/dr.html', dr=dr, pays=pays, people_ol=people_ol)
...
def edit(ku):
    ...
    try:
        db = get_db()
        db.execute(
            'update sp_dr set ku_ol = %s, event_year = %s, price = %s where ku = %s',
            (ku_ol, event_year, price, ku)
        )
        db.commit()
